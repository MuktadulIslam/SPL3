...
FILES = ['EQ.xlsx', 'JDT.xlsx', 'LC.xlsx', 'ML.xlsx', 'PDE.xlsx']

def CORAL(source_data, target_data, feature_number):
    X_s = source_data.iloc[:, :feature_number].values
    X_t = target_data.iloc[:, :feature_number].values
...
def save_results_to_csv(results, csv_filename):
    ...
    results_df['Accuracy'] = results_df['Accuracy'].round(4)  # Round accuracy to 4 decimal places
    results_df.to_csv(csv_filename, index=False)

# Values of FEATURE_NUMBER to iterate over
feature_numbers = [2, 5, 10, 20]
results = []
