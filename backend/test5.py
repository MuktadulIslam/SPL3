...
if __name__ == '__main__':
    ...
    src = pd.read_excel('EQ.xlsx')
    tar = pd.read_excel('JDT.xlsx')
    # src = pd.read_csv('EQ.csv')
    # tar = pd.read_csv('LC.csv')

    # Assuming the last column is the label
    Xs, Ys = src.iloc[:, :-1].values, src.iloc[:, -1].values
    Xt, Yt = tar.iloc[:, :-1].values, tar.iloc[:, -1].values

    coral = CORAL()
    acc, ypre = coral.fit_predict(Xs, Ys, Xt, Yt)
    print(f'Accuracy: {acc}')
    print(f'Predicted labels: {ypre}')
